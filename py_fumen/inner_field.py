...
class PlayField():
    ...
    def set_at(self, index: int, piece: Piece):
        if index < 0 or index >= len(self.__pieces):
...
class InnerField():
    ...
    def can_fill_all(self, positions: List[XY]) -> bool:
        return all(
            0 <= position.x < 10 and 0 <= position.y and position.y < FieldConstants.HEIGHT and self.get_number_at(position.x, position.y) is Piece.EMPTY for position in positions)
